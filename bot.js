...
bot.action("show_rules", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "⚠️ *Главное правило:* Физическое присутствие на локациях — обязательно! Бот отслеживает время ваших ответов. Слишком быстрые ответы караются штрафными баллами.\n\n" +
    "💡 *Как играть:*\n\n" +
    "1️⃣ *Регистрация команды*\n\n" +
    "• Нажмите /start\n" +
    "• Выберите название команды из предложенного списка (например, Хабенские, Клуб 'Красная таблетка', Безумцы с попкорном).\n" +
    "• Введите имена всех участников через запятую (например: Тони Старк, Брюс Уэйн, Питер Паркер).\n\n" +
    "2️⃣ *Выбор и прохождение точки*\n\n" +
    "• Ожидайте старта от админов: вы увидите сообщение «🚀 Игра началась! Можете приступать к прохождению точек!»\n" +
    "• В главном меню нажмите «🌍 Выбрать точку».\n" +
    "• Бот предложит список доступных локаций. Выбирайте любую! (После правильного прохождения локация пропадает из списка).\n" +
    "• Вы получите:\n" +
    "- Описание точки и её историю.\n" +
    "- Загадку-подсказку для поиска кода (код — это слово, цифра или фраза, физически находящаяся на локации).\n" +
    "- Кнопки для навигации в Google Maps, Яндекс.Картах, 2GIS и Telegram maps.\n" +
    "• Ваша задача: Найти на локации физический код (надпись, цифру, предмет), внимательно изучив её.\n" +
    "• Найдя код, просто введите его текстом в чат (Если в вопросе ожидается числовой ответ, мы уточним, следует ли указывать его прописью или цифрой, а также предоставим пример оформления ответа, если будут сложности в понимании. Есть обратная связь, не забывайте!)\n\n" +
    ...
    "- Текстовый → введите ответ вручную.\n" +
    "• *Система начисления очков (адаптивная):*\n" +
    "- Правильный ответ: Награда составляет от 1 до 10 баллов за вопрос.\n" +
    "- Слишком быстрый ответ (< n секунд): -3 балла к стоимости вопроса. Находитесь на локации, не пытайтесь угадать! Бот фиксирует время с момента активации точки.\n" +
    "- Неправильный ответ на вопрос: -1 балл. Можно пробовать отвечать на тот же вопрос снова, но его \"стоимость\" будет снижаться.\n" +
    "- Неверный код локации: -1 балл за каждую неудачную попытку.\n" +
    "- Минимальная награда за вопрос: 1 балл (даже после всех штрафов).\n\n" +
    "📊 *Контроль и помощь*\n\n" +
    "• «📊 Прогресс»: Ваш счёт, пройденные точки, время в игре.\n" +
    "• «🏆 Топ команд»: Рейтинг всех участников.\n" +
    "• «🏆 Мои призы»: Посмотреть полученные вашей командой промокоды (если вы их заработали).\n" +
    "• «ℹ️ Информация»: Полные правила, контакты организаторов, карта всех точек, ссылка на сайт.\n\n" +
    "🏆 *Система призов*\n\n" +
    "За первое прохождение определенного количества точек (4, 8, 10) ваша команда может получить промокод на заказ в кафе (например, в «Чизкейне» или «ProTesto»). Приз получает только первая команда, достигшая рубежа! Следите за своим прогрессом.\n\n" +
    "💡 *Важные советы от бота:*\n\n" +
    "• Не спешите! Наслаждайтесь поиском — спешка = штрафы. Изучайте локацию.\n" +
    "• Внимательно читайте описание и загадку — в них всегда есть прямые подсказки к коду.\n" +
    "• Думайте логически. Ответ на вопрос часто зависит от деталей локации или темы киноклуба.\n" +
    "• Работайте в команде! Обсуждайте, ищите вместе.\n" +
    "• «You know nothing, Jon Snow...» — но вы узнаете много нового о кино и Севастополе!\n\n" +
    "🆘 *Поддержка*\n\n" +
    "Если что-то пошло не так или вы заблудились:\n" +
    "• Телефон: \n" +
    "+7 (978) 7975 939 - Стас\n" +
    "+7 (978) 7363 212 - Настенька\n" +
    "• Telegram разработчика бота: @GeekLS \n" +
    "• Сайт: https://ulysses-club.github.io/odissea/\n\n" +
    "🌟 *Удачи в прохождении! Пусть сила (и хорошее кино) будут с вами!* 🎥\n" +
    "«Жизнь — это как коробка шоколадных конфет... а этот квест — как самый вкусный из них!»",
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [{ text: "⬅️ Назад", callback_data: "back_to_info" }]
